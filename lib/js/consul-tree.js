    ...
    function importConsul(cas, consulUrl, backendStatus) {
        ...
        fr.onload = function (e) {
            ...
            $.ajax({
                method: "POST",
                url: returnBackend(backendStatus),
                data: {
                    path: consulUrl,
                    method: "BulkIMPORT",
                    cas: cas,
    ...
    function updateFieldsToggle(readonly, userRights) {
    ...
    function dataValidation(data, backendStatus) {
        ...
        if (tobeFixedData.length > 0) {
            ...
            fixTree(tobeFixedData, backendStatus);
            status = false;
        } else {
    ...
    function fixTree(data, backendStatus) {
        if (data !== undefined) {
    ...
    function renDupNode(consulUrl, backendStatus) {
    ...
    function getTree(tree, consulUrl, backendStatus, allKeys) {
        $.ajax({
            method: "GET",
            url: returnBackend(backendStatus),
            dataType: 'json',
            ...
            if (data['data'] === '[]') {
                ...
                if (dataValidation(realData, backendStatus)) {
    ...
    function getValue(path, obj, consulUrl, userRights, backendStatus) {
    ...
    function ccPaste(path, parentId, replace, ccType, srcConsul, cas, consulUrl, backendStatus) {
        $('#overwriteModalId').modal('hide');
        $('#processingMdlID').modal({
            backdrop: 'static',
            keyboard: false
    ...
    function sendToConsul(path, value, reload, updateBtn, consulUrl, userRights, backendStatus) {
        path = path.replace(/\\/g, '/');
    ...
    function deleteNode(path, consulUrl, backendStatus) {
        $.ajax({
            method: "POST",
            url: returnBackend(backendStatus),
            data: {
    ...
    function exportConsul(data, backendStatus, consulTitle, consulUrl) {
...
function returnBackend(backendStatus){
    if (backendStatus === 'false'){
        ...
        return backendStatus + "requests.php";
    }
}