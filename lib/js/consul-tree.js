...
function getSetConfig(modify) {
    ...
    if (localStorage['selectedConsul']) {
        ...
        SelectedConsul = JSON.stringify(stringedObj);
        localStorage['selectedConsul'] = SelectedConsul;
        localStorage['consulUrl'] = stringedObj.url;
    }

    consulUrlSelector.val(stringedObj.url).attr("selected", "selected");

    $('#consulFullUrlId').text(stringedObj.url);
    $('#consulTitleID').text(stringedObj.title);
    return stringedObj;
}
