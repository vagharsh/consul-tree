...
function importConsul(cas, consulUrl, backendPath) {
...
function dataValidation(data, backendPath, consulUrl) {
...
function fixTree(data, backendPath, consulUrl) {
    if (data !== undefined) {
...
function getConsulLocations(consul){
    var optionElements = '';
...
function renDupNode(consulUrl, backendPath) {
...
function getTree(tree, consulUrl, backendPath, allKeys) {
    $.ajax({
        method: "GET",
        url: backendPath,
        dataType: 'json',
...
function getValue(path, obj, consulUrl, userRights, backendPath) {
...
function ccPaste(path, parentId, replace, ccType, srcConsul, cas, consulUrl, backendPath) {
    $('#overwriteModalId').modal('hide');
    $('#processingMdlID').modal({
        backdrop: 'static',
        keyboard: false
...
function sendToConsul(path, value, reload, updateBtn, consulUrl, userRights, backendPath) {
    path = path.replace(/\\/g, '/');
...
function deleteNode(path, consulUrl, backendPath) {
    $.ajax({
        method: "POST",
        url: backendPath,
        data: {
...
function exportConsul(data, backendPath, consulTitle, consulUrl) {
...
function customMenu(node, userRights, consulUrl, backendPath) {
