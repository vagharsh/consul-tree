...
/*jshint -W083 */
	...
	$.jstree.core.prototype = {
		...
		create_node : function (par, node, pos, callback, is_loaded) {
			...
			if(!node) { node = { "text" : this.get_string('new_folder') }; }
			if(typeof node === "string") {
				node = { "text" : node };
			} else {
				node = $.extend(true, {}, node);
			}
			if(node.text === undefined) { node.text = this.get_string('new_folder'); }
			var tmp, dpc, i, j;
 ...
 */
	...
	$.jstree.defaults.dnd = {
		...
		/**
		 * controls whether use HTML5 dnd backend instead of classical. That will allow better integration of dnd events with other HTML5 controls.
