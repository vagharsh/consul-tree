    ...
    fr.onload = function (e) {
        ...
        $.each(importedFileC, function (key, value) {
            ...
            $.ajax({
                ...
                $("#summaryTableId tbody").append(markup);
            });
        });
    };
...
function sendToConsul(path, value, reload, updateBtn) {
    ...
    var fullPath = getFromLocalStorage('consulUrl') + path;
...
function exportConsul(data) {
    var selectedConsul = getFromLocalStorage('selectedConsul'),
        consulTitle = selectedConsul['title'],
        exportFileNameObj = $('#exportFileName'),filename,
