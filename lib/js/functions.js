...
function exportConsul(data) {
    var receivedData = {}, receiveCount = 0,
        i,j, percent, allCount = 0, arrayChunk,chunk = 20;
    ...
    for (i=0,j=data.length; i<j; i+=chunk) {
        arrayChunk = data.slice(i,i+chunk);
        $.ajax({
            method: "POST",
            url: "backend/requests.php",
            dataType: "json",
            data: {
                consul: getFromLocalStorage('consulUrl'),
                method: "EXPORT",
                path: JSON.stringify(arrayChunk)
            }
        }).done(function (data) {
            $.extend( receivedData, data );
            receiveCount++;
            percent = receiveCount * 100 / allCount;
            updateProgress(Math.round(percent));
            if (allCount === receiveCount){
                pushToDownload(receivedData);
            }
        })
    }
}
